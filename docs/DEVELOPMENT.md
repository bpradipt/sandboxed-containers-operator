...
## Installating the Operator using OpenShift Web console 
...
spec:
 ...
 sourceType: grpc
 image:  quay.io/user/openshift-sandboxed-containers-operator-catalog:v0.0.1
 updateStrategy:
   registryPoll:
      interval: 5m
...
## Installating the Operator using CLI

Ensure `KUBECONFIG` points to the target OpenShift cluster
```
...
When deploying the Operator using CLI, cert-manager is automatically installed otherwise
