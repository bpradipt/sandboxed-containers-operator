...
The configuration for the K8s jobs are provided via either
``aws-podvm-image-cm`` configMap for AWS or `azure-podvm-image-cm` configMap
for Azure respectively.
...
For Azure, by default the job deletes the pod VM gallery hosting the image as
well. This is to ensure that if the pod VM image and gallery is created by the
OSC operator during `kataConfig` creation, then the same resources will be
deleted during `kataConfig` deletion.

If you are using the job manifest for manually deleting a pod VM image (eg. `oc
apply -f osc-podvm-image-deletion.yaml`), and you don't want the gallery to be
deleted then change the command string in the job manifest as shown below:

~~command: ["/podvm-builder.sh", "delete", "-f", "-g"]~~ => original command string

```yaml
command: ["/podvm-builder.sh", "delete", "-f"]  => modified command string
```
...
* For Azure, if the `IMAGE_GALLERY_NAME` key in the `azure-podvm-image-cm`
  ...
  `IMAGE_GALLERY_NAME` key with the pattern `PodVMGallery_$clusterId`. Note
  that only the first 8 chars of the OCP `clusterId` is used. The `clusterID`
  is also added as a tag to the OSC created gallery (eg.  `clusterID=12345678`)
...
* For Azure, the gallery is also deleted (by force). Before deletion the tag
  (eg. clusterID=12345678) is matched for the gallery
